...
class file_ext
{
                ...
                if(is_array($value)){
                    foreach ($value as $keys => $item) {
                        if(!empty($item)){
                        	...
                        	if(strstr($key,"time") || strstr($key,"Time")){
                        		$time = strtotime($item);
                            	$str .= $keys." '$time'";
                        	}else{
                        		$str .= $keys." '$item'";
	...
	// 添加搜索历史
	public static function history_keyword($keyword='',$uid=0){
		$cache = cache::getClass();
        $arr = $cache->hGet('search:'.$uid,$uid);
        if($keyword != ''){
	        if($arr){
	        	...
	        	$cache->hSet('search:'.$uid,$uid,json_encode($array));
	        }else{
	        	$array = array($keyword);
	        	$cache->hSet('search:'.$uid,$uid,json_encode($array));
	        }
