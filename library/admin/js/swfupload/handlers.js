...
function attShow(serverData,file)
{
	if (serverData.substring(0, 7) === "FILEID:") {
		...
		var desc	= data[1];
		var src		= data[2];
		var box = '<br />URL：<input type="text" class="inp185" name="urls[]" />';
		var txtinput = '<br />文本：<input type="text" class="inp185" name="texts[]" />';
		...
		var img = '<div class="cancel-icon" onclick=" return attDel(this,\''+src+'\')"></div><table class="table table-striped"><tr align="left"><td width="180">'+input+'<img src="'+siteUrl+thumb+'" title="'+desc+'" /></td>' +
			'<td width="250">'+box+'</td><td width="250">'+txtinput+'</td></tr></table>';
...
{
	$("#media_path").val(location.origin+'/'+serverData);
	$("#mp").val(location.origin+'/'+serverData);
	var progress = new FileProgress(file, this.customSettings.progressTarget);
	progress.setComplete();
	progress.setStatus("文件上传成功");
	alertMsg('上传成功！');
